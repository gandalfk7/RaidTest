...
for READ_AHEAD in 1536 4096 32768 262144 524288; do
	...
	for STRIPE_CACHE_SIZE in 512 1024 2048 4096 8192 16384 32768; do
		...
		RES_WRITE=`dd if=/dev/zero of=$FILE bs=1M count=$FILESIZE conv=fdatasync  2>&1 | awk '/bytes/{print $(NF-1), $NF}'`
		sync
		RES_READ=`dd if=$FILE of=/dev/null bs=1M 2>&1 | awk '/bytes/{print $(NF-1), $NF}'`
